...
### Phase 0 — Bootstrap (0.5d)
- Create `homedrive/` with the layout above.
- Update `go.work`.
- `go.mod` with `fsnotify`, `cobra`, YAML loader (match `myhome`'s loader),
  `paho.mqtt.golang`, `bbolt`, `doublestar`.
- Skeleton `cmd/homedrive/main.go` with subcommands `run`, `ctl status`,
  `ctl pause`, `ctl resume`, `ctl resync`.
- Global `--dry-run` flag wired through context and respected by the rclone
  wrapper.
- Makefile target `make build-homedrive`, plus `test-linux` (OrbStack).
- Initial README + this `PLAN.md`.
